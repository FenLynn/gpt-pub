...
func main() {
    ...
    if missingDotnet || missingWebView {
        ...
        text := "检测到缺少以下运行组件：\n\n• " + strings.Join(names, "\n• ") +
            "\n\n是否立即从微软官方下载并安装？安装结束后工作台会自动启动。"
        if messageBox("Personal Workbench", text, mbYesNo|mbIconInformation|mbTopMost) != idYes {
        ...
        if !hasDotNetDesktop8() || !hasWebView2Runtime() {
            ...
            messageBox("Personal Workbench", "安装程序已结束，但仍未检测到所需运行组件。请重新启动电脑后再试；若仍失败，请手动安装 .NET 8 Desktop Runtime 与 WebView2 Runtime。", mbOK|mbIconError|mbTopMost)
            return
        }
    }
    ...
    if err != nil {
        ...
        messageBox("Personal Workbench", "工作台程序释放失败：\n\n"+err.Error(), mbOK|mbIconError|mbTopMost)
        return
    }
    ...
    if err := cmd.Start(); err != nil {
        ...
        messageBox("Personal Workbench", "工作台启动失败：\n\n"+err.Error(), mbOK|mbIconError|mbTopMost)
        return
    }
...
func installMissingDependencies(dotnet, webview bool) error {
    tempDir, err := os.MkdirTemp("", "PersonalWorkbench-Setup-")
    if err != nil {
    ...
    if dotnet {
        messageBox("Personal Workbench", "即将下载并安装 .NET 8 Desktop Runtime。下载期间请稍候，随后可能出现 Windows 管理员确认窗口。", mbOK|mbIconInformation|mbTopMost)
    ...
    if webview {
        messageBox("Personal Workbench", "即将下载并安装 WebView2 Runtime。下载期间请稍候，随后可能出现 Windows 管理员确认窗口。", mbOK|mbIconInformation|mbTopMost)
...
func downloadFile(client *http.Client, url, destination string) error {
    ...
    req.Header.Set("User-Agent", "PersonalWorkbench/"+appVersion)
...
func extractApplication() (string, error) {
    ...
    if err != nil {
        return "", errors.New("安装包中缺少 PersonalWorkbench.App.exe")
    }
    ...
    target := filepath.Join(root, "PersonalWorkbench.App.exe")
...
func logLine(message string) {
    ...
    dir := filepath.Join(appData, "PersonalWorkbench", "logs")
