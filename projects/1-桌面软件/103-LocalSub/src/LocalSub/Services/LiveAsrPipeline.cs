...
public sealed class LiveAsrPipeline : IAsyncDisposable
{
    ...
    public async Task StartAllAudioAsync(AppSettings settings, ModelDescriptor model, ModelManager models, IProgress<ModelOperationProgress>? runtimeProgress = null, CancellationToken ct = default)
    {
        await StopAsync();
        ...
        if (!model.Id.StartsWith("streaming-paraformer-", StringComparison.OrdinalIgnoreCase))
            throw new NotSupportedException("实时字幕当前请使用 Streaming Paraformer 模型。SenseVoice 的模拟流式将在后续单独接入。 ");
        ...
        try
        {
            StatusChanged?.Invoke("启动 Windows 输出音频捕获");
            _allAudio.Start();
            _running = true;
