...
# this v4.5.3 stability release with a temporary exact appVersion change,
...
$newVersionLine = 'const appVersion = "4.5.3"'
...
$runtimeReadme = @"
Mediova v$version Stability Runtime
...
This is a v4.5.3 stability release; it does not change the official v4.5.0 tag or Release.
"@
...
if ($PackageZip) {
    $zip = Join-Path $buildRoot "Mediova-v$version-Stability-Runtime.zip"
    Compress-Archive -Path (Join-Path $runtimeRoot "*") -DestinationPath $zip -CompressionLevel Optimal -Force
    $zipHash = (Get-FileHash $zip -Algorithm SHA256).Hash.ToLowerInvariant()
    $hashLines += "$zipHash  Mediova-v$version-Stability-Runtime.zip"
