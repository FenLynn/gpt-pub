...
当前稳定候选基线：**v0.4.10 Maintenance Baseline**。当前 `p107-exp` 正在开发 **v0.4.22 Top-only Motion / Metric Frame Lock**，尚未提升 stable。
...
- Web UI 已有 Dashboard、模块独立页、Data 和 Settings。v0.4.22 继续锁定 Dashboard：仅顶栏四个采集选择图标动态；模块标题图标固定；72×72/48 帧慢循环 GIF；REC 与相机完全共用单次反馈；顶部 metric 贴 Plot 上框并按几何中点放 separator；旧 WinForms 的模块拖出浮窗/拖回嵌入仍未恢复，不得描述为已完成。
...
- 顶部采集选择图标与模块左上图标共享 selected 绿色状态，但只有顶栏四个选择图标允许 GIF 动画；模块标题图标固定静态。
