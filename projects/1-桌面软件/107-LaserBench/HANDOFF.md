...
当前稳定候选基线：**v0.4.10 Maintenance Baseline**。当前 `p107-exp` 正在开发 **v0.4.15 Dashboard-first Correction**，尚未提升 stable。
...
- Web UI 已有 Dashboard、模块独立页、Data 和 Settings。v0.4.15 当前重点回到 Dashboard：标题/刻度可读性、下半区空间、Big Readout 紧凑度、截图层级和录像时长显示；旧 WinForms 的模块拖出浮窗/拖回嵌入仍未恢复，不得描述为已完成。
...
- Dashboard 关键结果支持单实例 Big Readout，点击新指标复用同一监视窗，可拖动和右下角等比例缩放；窗口尽量包裹读数，不保留大块无效空白。
- 顶部采集选择图标与模块左上图标联动：选中绿色，running 时绿色呼吸。
