...
当前开发版本为 **v0.4.24（p107-exp）**，稳定候选仍为 v0.4.10，等待本轮 Windows 实机视觉验收后再决定是否提升。v0.4.24 保持四宫格外部位置不动：废弃外部 GIF 动图，顶部四个采集图标改成 WebView2 原生内联 SVG/SMIL 动画并由 GUI smoke 验证时间轴实际前进；模块左上图标继续静态。相机/REC 统一 hover 浮起；M²、顶部结果背景、Y 轴端点刻度和 Power 左右双手柄范围窗均保留。
