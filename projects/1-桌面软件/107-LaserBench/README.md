...
当前开发版本为 **v0.4.20（p107-exp）**，稳定候选仍为 v0.4.10，等待本轮 Windows 实机视觉验收后再决定是否提升。v0.4.20 保持四宫格外部位置不动：选中采集源直接播放 GIF，去掉采集勾和 idle“就绪”，统一单位基线，重做 Power 左→右历史增长与 overview，并增强 REC hover 触发反馈：冷深蓝灰壳体、仅绘图区白底、模块标题/坐标标题/关键读数贴边悬浮，低频参数移入对应模块页，并新增单实例 Big Readout 远距监视。真实厂商硬件仍留给 0.5.x 按设备抽象层逐项接入。
...
- 顶部采集选择图标与对应模块左上角图标联动：选中后直接播放内置 GIF——Power 四 bar 跳动、Spectrum/Scope 横向波形扫动、Beam 保留 target 图标语义并向外脉冲；旧的选中勾不再显示。
...
v0.4.20 仍默认启用 Simulator。真实仪器从 0.5.x 起按最小闭环逐个接入：
