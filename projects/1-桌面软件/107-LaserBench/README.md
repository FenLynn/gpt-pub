...
当前开发版本为 **v0.4.22（p107-exp）**，稳定候选仍为 v0.4.10，等待本轮 Windows 实机视觉验收后再决定是否提升。v0.4.22 保持四宫格外部位置不动：只有顶部四个采集选择图标动态，模块左上图标固定；GIF 升级为 72×72 / 48 帧 / 4.8 s 循环；REC 直接复用相机单次按钮反馈；顶部结果贴近 Plot 上框线并把分隔线放到指标间隙中点，重点修正 M²：冷深蓝灰壳体、仅绘图区白底、模块标题/坐标标题/关键读数贴边悬浮，低频参数移入对应模块页，并新增单实例 Big Readout 远距监视。真实厂商硬件仍留给 0.5.x 按设备抽象层逐项接入。
...
v0.4.22 仍默认启用 Simulator。真实仪器从 0.5.x 起按最小闭环逐个接入：
