...
当前开发版本为 **v0.4.18（p107-exp）**，稳定候选仍为 v0.4.10，等待本轮 Windows 实机视觉验收后再决定是否提升。v0.4.18 保持四宫格外部位置不动，只精修采集动态图标、Beam 内部比例、标题行信息密度、Scope 双图间隙与 REC 状态条：冷深蓝灰壳体、仅绘图区白底、模块标题/坐标标题/关键读数贴边悬浮，低频参数移入对应模块页，并新增单实例 Big Readout 远距监视。真实厂商硬件仍留给 0.5.x 按设备抽象层逐项接入。
...
- 顶部采集选择图标与对应模块左上角图标联动：选中变绿，实际采集中执行模块语义动画——Power 四条 bar 跳动、Spectrum/Scope 波形横向扫动、Beam 同心环向外扩散。
...
v0.4.18 仍默认启用 Simulator。真实仪器从 0.5.x 起按最小闭环逐个接入：
