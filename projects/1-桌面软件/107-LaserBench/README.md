...
当前开发版本为 **v0.4.19（p107-exp）**，稳定候选仍为 v0.4.10，等待本轮 Windows 实机视觉验收后再决定是否提升。v0.4.19 保持四宫格外部位置不动：采集动画改为内置 GIF，顶部数据进一步统一字号/右边界/分隔，REC 垂直居中，并修复 Beam Z/Att 滑条回写跳变：冷深蓝灰壳体、仅绘图区白底、模块标题/坐标标题/关键读数贴边悬浮，低频参数移入对应模块页，并新增单实例 Big Readout 远距监视。真实厂商硬件仍留给 0.5.x 按设备抽象层逐项接入。
...
- 顶部采集选择图标与对应模块左上角图标联动：选中变绿；实际采集中调用内置 GIF——Power 四 bar 跳动、Spectrum/Scope 横向波形扫动、Beam 保留 target 图标语义并向外脉冲。
...
v0.4.19 仍默认启用 Simulator。真实仪器从 0.5.x 起按最小闭环逐个接入：
