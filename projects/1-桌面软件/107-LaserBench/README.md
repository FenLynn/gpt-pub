...
当前开发版本为 **v0.4.23（p107-exp）**，稳定候选仍为 v0.4.10，等待本轮 Windows 实机视觉验收后再决定是否提升。v0.4.23 保持四宫格外部位置不动：重做浏览器兼容 GIF 并让 GUI smoke 验证 WebView2 实际解码；相机/REC 统一为 hover 浮起、录制中不闪；平均 M² 简化为 M²；结果条增加轻微背景场；修正所有 Y 轴端点刻度裁切；Power overview 改为真正左右双手柄显示窗口：冷深蓝灰壳体、仅绘图区白底、模块标题/坐标标题/关键读数贴边悬浮，低频参数移入对应模块页，并新增单实例 Big Readout 远距监视。真实厂商硬件仍留给 0.5.x 按设备抽象层逐项接入。
...
v0.4.23 仍默认启用 Simulator。真实仪器从 0.5.x 起按最小闭环逐个接入：
