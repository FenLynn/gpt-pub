...
v0.4.22 继续采用 Graph-first Dashboard；已定型的四宫格外部位置保持不动，只修顶部采集动态图、结果条与 Plot 上框关系、REC 单次反馈和 M² 分隔：
...
顶栏采集 GIF 固定为高分辨率、多帧、慢周期资源：当前 72×72、48 帧、4.8 s 循环。模块左上标题图标不参与动画。顶部结果条贴近下方 Plot 数据框上沿，相邻指标的分隔线位于固定间隙的几何中点，M² 不使用单独例外排版。
