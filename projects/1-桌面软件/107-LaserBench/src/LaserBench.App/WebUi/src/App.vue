...
<div v-else class="module-pane-scroll results-pane"><div class="result-section"><h4>光谱结果</h4><div class="result-row"><span>中心波长</span><b>{{snapshot.spectrum.centerWavelength.toFixed(4)}} nm</b></div><div class="result-row"><span>3 dB 线宽</span><b>{{snapshot.spectrum.linewidth3Db.toFixed(4)}} nm</b></div><div class="result-row"><span>RMS 线宽</span><b>{{snapshot.spectrum.linewidthRms.toFixed(4)}} nm</b></div><div class="result-row"><span>积分功率</span><b>{{snapshot.spectrum.power.toFixed(3)}} dBm</b></div></div><div class="result-section"><h4>扫描参数</h4><div class="result-row"><span>波长范围</span><b>{{settingsDraft.osaStart}} – {{settingsDraft.osaStop}} nm</b></div><div class="result-row"><span>分辨率带宽</span><b>{{settingsDraft.osaResolution}} nm</b></div><div class="result-row"><span>灵敏度</span><b>{{settingsDraft.osaSensitivity==='LOW'?'低':settingsDraft.osaSensitivity==='MID'?'中':settingsDraft.osaSensitivity==='HIGH1'?'高 1':'高 2'}}</b></div><div class="result-row"><span>平均次数</span><b>{{settingsDraft.osaAverage}} 次</b></div><div class="result-row"><span>扫描模式</span><b>{{settingsDraft.osaSweepMode==='REPEAT'?'重复':'单次'}}</b></div><div class="result-row"><span>参考电平</span><b>{{settingsDraft.osaRefLevel}} dBm</b></div><div class="result-row"><span>垂直刻度</span><b>{{settingsDraft.osaDbPerDiv}} dB/div</b></div><div class="result-row"><span>参考光谱 / 峰值标记</span><b>{{settingsDraft.osaShowRef?'显示':'隐藏'}} / {{settingsDraft.osaMarkerPeak?'显示':'隐藏'}}</b></div></div><div class="result-section"><h4>接口状态</h4><div class="result-row"><span>当前数据源</span><b>{{snapshot.mode==='SIM'?'模拟器':'真实硬件'}}</b></div><div class="result-row"><span>目标设备</span><b>{{interfaceFor('spectrum')?.deviceName}}</b></div><div class="result-row"><span>通信接口</span><b>{{interfaceFor('spectrum')?.interfaceName}}</b></div><div class="result-row"><span>接口地址</span><b>{{interfaceFor('spectrum')?.endpoint}}</b></div><div class="result-row"><span>配置状态</span><b>{{interfaceStateText(interfaceFor('spectrum')?.state)}}</b></div><p>{{interfaceFor('spectrum')?.message}}</p></div></div></div></aside>
