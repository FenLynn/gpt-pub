...
<label class="config-row"><span>本地显示偏移（nm）</span><input type="number" step="0.001" v-model.number="settingsDraft.osaWavelengthOffsetNm" @change="saveSettings"/></label>
<label class="config-row"><span>波长基准</span><select v-model="settingsDraft.osaWavelengthReference" @change="saveSettings"><option value="AIR">空气</option><option value="VACUUM">真空</option></select></label><p class="config-note">RBW、灵敏度、平均、扫描模式、采样点数、轨迹模式与波长基准会写入 AQ6370D；本地平滑和显示偏移只影响 LaserBench 显示，不改写仪器校准。</p></div>
