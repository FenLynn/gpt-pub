...
const spectrumPageSeries = computed(()=>snapshot.value.spectrum.traces
  ...
  .map(trace=>({...trace,points:limitSpectrum(smoothSpectrum(trace.points,settingsDraft.value.osaSmoothingPoints),settingsDraft.value.osaSamplePoints)
    .map(p=>({x:p.x+settingsDraft.value.osaWavelengthOffsetNm,y:p.y}))})))
const osaDisplayCenter = computed(()=>snapshot.value.spectrum.centerWavelength+settingsDraft.value.osaWavelengthOffsetNm)
          ...
          <div class="module-head power-head graph-overlay">
            ...
            <template v-if="activePage==='power'">
              ...
              <div class="power-channel-board">
                <div><span v-for="n in 4" :key="'p-slot-'+n"><i class="channel-light" :class="{online:n<=powerConnectedCount}"></i>power{{n}}</span></div>
                <div><span v-for="n in 4" :key="'m-slot-'+n"><i class="channel-light" :class="{online:n<=mathChannelCount}"></i>math{{n}}</span></div>
              </div>
            </template>
        ...
        <article class="instrument-panel spectrum-panel" :class="{hidden:!['dashboard','spectrum'].includes(activePage)}">
          <div class="spectrum-head graph-overlay">
            ...
            <template v-if="activePage==='spectrum'">
              <div class="focus-head-metrics">
                <div class="focus-head-value"><span>中心波长</span><div><b>{{(activePage==='spectrum'?osaDisplayCenter:snapshot.spectrum.centerWavelength).toFixed(2)}}</b></div></div>
                <div class="focus-head-value"><span>3 dB 线宽</span><div><b>{{snapshot.spectrum.linewidth3Db.toFixed(2)}}</b></div></div>
                <div class="focus-head-value"><span>RMS 线宽</span><div><b>{{snapshot.spectrum.linewidthRms.toFixed(2)}}</b></div></div>
                <div class="focus-head-value"><span>积分功率</span><div><b>{{snapshot.spectrum.power.toFixed(1)}}</b></div></div>
              </div>
            ...
            <div v-else class="osa-metrics"><button class="metric-trigger primary-metric" @click="openReadout('spectrumCenter')"><em>λc</em><span class="metric-value-chip"><b>{{(activePage==='spectrum'?osaDisplayCenter:snapshot.spectrum.centerWavelength).toFixed(2)}}</b></span></button><button class="metric-trigger secondary-trigger" @click="openReadout('spectrum3db')"><em>3 dB</em><span class="metric-value-chip"><b>{{snapshot.spectrum.linewidth3Db.toFixed(2)}}</b></span></button><button class="metric-trigger secondary-trigger" @click="openReadout('spectrumRms')"><em>RMS</em><span class="metric-value-chip"><b>{{snapshot.spectrum.linewidthRms.toFixed(2)}}</b></span></button><button class="metric-trigger primary-metric" @click="openReadout('spectrumPower')"><em>P</em><span class="metric-value-chip"><b>{{snapshot.spectrum.power.toFixed(1)}}</b></span></button></div>
          </div>

          <div v-if="activePage==='spectrum'" class="focus-summary">
            <div class="focus-metric"><span>中心波长 λc</span><div><b>{{(activePage==='spectrum'?osaDisplayCenter:snapshot.spectrum.centerWavelength).toFixed(2)}}</b></div></div>
            <div class="focus-metric"><span>3 dB 线宽</span><div><b>{{snapshot.spectrum.linewidth3Db.toFixed(2)}}</b></div></div>
            <div class="focus-metric"><span>RMS 线宽</span><div><b>{{snapshot.spectrum.linewidthRms.toFixed(2)}}</b></div></div>
            <div class="focus-metric"><span>积分功率</span><div><b>{{snapshot.spectrum.power.toFixed(1)}}</b></div></div>
          </div>
...
<label class="config-row"><span>视频带宽（Hz）</span><select v-model.number="settingsDraft.osaVideoBandwidthHz" @change="saveSettings"><option :value="10">10</option><option :value="100">100</option><option :value="1000">1k</option><option :value="10000">10k</option><option :value="100000">100k</option></select></label></div>
<div class="config-section"><h4>轨迹 / 处理</h4>
...
<label class="config-row"><span>平滑点数</span><select v-model.number="settingsDraft.osaSmoothingPoints" @change="saveSettings"><option :value="1">关闭</option><option :value="3">3</option><option :value="5">5</option><option :value="9">9</option><option :value="15">15</option></select></label>
...
<label class="config-row"><span>波长基准</span><select v-model="settingsDraft.osaWavelengthReference" @change="saveSettings"><option value="AIR">空气</option><option value="VACUUM">真空</option></select></label></div>
...
<div class="config-row switch"><span>自动峰值搜索</span><button type="button" class="toggle-pill" :class="{on:settingsDraft.osaAutoPeakSearch}" @click="flipSetting('osaAutoPeakSearch')"><span>{{settingsDraft.osaAutoPeakSearch?'开':'关'}}</span></button></div>
<label class="config-row"><span>峰值阈值（dB）</span><input type="number" min="0" max="80" step="0.5" v-model.number="settingsDraft.osaPeakThresholdDb" @change="saveSettings"/></label></div><div class="config-section"><h4>设备接口</h4><div class="config-row switch"><span>启用真实接口</span><button type="button" class="toggle-pill" :class="{on:settingsDraft.spectrumInterfaceEnabled}" @click="flipSetting('spectrumInterfaceEnabled')"><span>{{settingsDraft.spectrumInterfaceEnabled?'开':'关'}}</span></button></div><div class="config-static"><span>目标设备</span><b>Yokogawa AQ6370D</b></div><label class="config-row"><span>接口地址</span><input type="text" v-model="settingsDraft.spectrumInterfaceEndpoint" @change="saveSettings"/></label></div></div>
<div v-else class="module-pane-scroll results-pane"><div class="result-section"><h4>光谱结果</h4><div class="result-row"><span>中心波长</span><b>{{snapshot.spectrum.centerWavelength.toFixed(4)}} nm</b></div><div class="result-row"><span>3 dB 线宽</span><b>{{snapshot.spectrum.linewidth3Db.toFixed(4)}} nm</b></div><div class="result-row"><span>RMS 线宽</span><b>{{snapshot.spectrum.linewidthRms.toFixed(4)}} nm</b></div><div class="result-row"><span>积分功率</span><b>{{snapshot.spectrum.power.toFixed(3)}} dBm</b></div></div><div class="result-section"><h4>扫描参数</h4><div class="result-row"><span>波长范围</span><b>{{settingsDraft.osaStart}} – {{settingsDraft.osaStop}} nm</b></div><div class="result-row"><span>分辨率带宽</span><b>{{settingsDraft.osaResolution}} nm</b></div><div class="result-row"><span>采样点数</span><b>{{settingsDraft.osaSamplePoints}}</b></div><div class="result-row"><span>视频带宽</span><b>{{settingsDraft.osaVideoBandwidthHz}} Hz</b></div><div class="result-row"><span>轨迹模式</span><b>{{settingsDraft.osaTraceMode==='WRITE'?'写入':settingsDraft.osaTraceMode==='MAXHOLD'?'最大保持':'轨迹平均'}}</b></div><div class="result-row"><span>平滑点数</span><b>{{settingsDraft.osaSmoothingPoints}}</b></div><div class="result-row"><span>波长偏移</span><b>{{settingsDraft.osaWavelengthOffsetNm}} nm</b></div><div class="result-row"><span>波长基准</span><b>{{settingsDraft.osaWavelengthReference==='AIR'?'空气':'真空'}}</b></div><div class="result-row"><span>灵敏度</span><b>{{settingsDraft.osaSensitivity==='LOW'?'低':settingsDraft.osaSensitivity==='MID'?'中':settingsDraft.osaSensitivity==='HIGH1'?'高 1':'高 2'}}</b></div><div class="result-row"><span>平均次数</span><b>{{settingsDraft.osaAverage}} 次</b></div><div class="result-row"><span>扫描模式</span><b>{{settingsDraft.osaSweepMode==='REPEAT'?'重复':'单次'}}</b></div><div class="result-row"><span>参考电平</span><b>{{settingsDraft.osaRefLevel}} dBm</b></div><div class="result-row"><span>垂直刻度</span><b>{{settingsDraft.osaDbPerDiv}} dB/div</b></div><div class="result-row"><span>参考光谱 / 峰值标记</span><b>{{settingsDraft.osaShowRef?'显示':'隐藏'}} / {{settingsDraft.osaMarkerPeak?'显示':'隐藏'}}</b></div><div class="result-row"><span>自动峰值搜索 / 阈值</span><b>{{settingsDraft.osaAutoPeakSearch?'开启':'关闭'}} / {{settingsDraft.osaPeakThresholdDb}} dB</b></div></div><div class="result-section"><h4>接口状态</h4><div class="result-row"><span>当前数据源</span><b>{{snapshot.mode==='SIM'?'模拟器':'真实硬件'}}</b></div><div class="result-row"><span>目标设备</span><b>{{interfaceFor('spectrum')?.deviceName}}</b></div><div class="result-row"><span>通信接口</span><b>{{interfaceFor('spectrum')?.interfaceName}}</b></div><div class="result-row"><span>接口地址</span><b>{{interfaceFor('spectrum')?.endpoint}}</b></div><div class="result-row"><span>配置状态</span><b>{{interfaceStateText(interfaceFor('spectrum')?.state)}}</b></div><p>{{interfaceFor('spectrum')?.message}}</p></div></div></div></aside>
          <PlotCanvas class="spectrum-plot" :series="activePage==='spectrum'?spectrumPageSeries:snapshot.spectrum.traces" :x-min="settingsDraft.osaStart" :x-max="settingsDraft.osaStop" :y-min="activePage==='spectrum'?osaYMin:-100" :y-max="activePage==='spectrum'?osaYMax:0" :vertical-marker="activePage==='spectrum'&&settingsDraft.osaMarkerPeak&&settingsDraft.osaAutoPeakSearch?osaDisplayCenter:undefined" x-label="波长 (nm)" y-label="功率 (dBm)" :show-axis-labels="activePage==='spectrum'" />
        </article>
...
<div class="config-section"><h4>位置 / 衰减</h4><label class="config-control"><span>Z 位置（mm）</span><input type="range" min="-24" max="24" step="0.1" :value="beamZ" @pointerdown="beginZDrag" @pointerup="endZDrag" @pointercancel="endZDrag" @input="onZInput"/><b>{{beamZ.toFixed(1)}} mm</b></label><label class="config-control"><span>衰减（dB）</span><input type="range" min="0" max="40" step="0.1" :value="beamAtt" @pointerdown="beginAttDrag" @pointerup="endAttDrag" @pointercancel="endAttDrag" @input="onAttInput"/><b>{{beamAtt.toFixed(1)}} dB</b></label><button class="config-action" :class="{active:beamPlaying}" @click="toggleBeamPlay">{{beamPlaying?'停止 Z 浏览':'自动 Z 浏览'}}</button></div>
<div class="config-section"><h4>M² 分析</h4><label class="config-row"><span>运行模式</span><select v-model="settingsDraft.beamRunMode" @change="saveSettings"><option value="AUTO">自动</option><option value="MANUAL">手动</option></select></label><label class="config-row"><span>束宽算法</span><select v-model="settingsDraft.beamWidthMethod" @change="saveSettings"><option value="D4SIGMA">D4σ / ISO 11146</option><option value="FWHM">FWHM</option></select></label><div class="config-row switch"><span>自动剔除离群点</span><button type="button" class="toggle-pill" :class="{on:settingsDraft.beamAutoOutlier}" @click="flipSetting('beamAutoOutlier')"><span>{{settingsDraft.beamAutoOutlier?'开':'关'}}</span></button></div><div class="config-row switch"><span>显示 X 拟合曲线</span><button type="button" class="toggle-pill" :class="{on:settingsDraft.beamShowX}" @click="flipSetting('beamShowX')"><span>{{settingsDraft.beamShowX?'开':'关'}}</span></button></div><div class="config-row switch"><span>显示 Y 拟合曲线</span><button type="button" class="toggle-pill" :class="{on:settingsDraft.beamShowY}" @click="flipSetting('beamShowY')"><span>{{settingsDraft.beamShowY?'开':'关'}}</span></button></div></div><div class="config-section"><h4>设备接口</h4><div class="config-row switch"><span>启用真实接口</span><button type="button" class="toggle-pill" :class="{on:settingsDraft.beamInterfaceEnabled}" @click="flipSetting('beamInterfaceEnabled')"><span>{{settingsDraft.beamInterfaceEnabled?'开':'关'}}</span></button></div><div class="config-static"><span>目标设备</span><b>Ophir Spiricon SP920 / BeamSquared</b></div><label class="config-row"><span>接口地址</span><input type="text" v-model="settingsDraft.beamInterfaceEndpoint" @change="saveSettings"/></label></div></div>
<div v-else class="module-pane-scroll results-pane"><div class="result-section"><h4>光束结果</h4><div class="result-row"><span>M² X</span><b>{{snapshot.beam.m2x.toFixed(4)}}</b></div><div class="result-row"><span>M² Y</span><b>{{snapshot.beam.m2y.toFixed(4)}}</b></div><div class="result-row"><span>M²</span><b>{{snapshot.beam.m2mean.toFixed(4)}}</b></div><div class="result-row"><span>束宽 X</span><b>{{snapshot.beam.spotWidthX.toFixed(2)}} μm</b></div><div class="result-row"><span>束宽 Y</span><b>{{snapshot.beam.spotWidthY.toFixed(2)}} μm</b></div><div class="result-row"><span>椭圆度</span><b>{{beamEllipticity.toFixed(4)}}</b></div></div><div class="result-section"><h4>分析参数</h4><div class="result-row"><span>Z 位置</span><b>{{beamZ.toFixed(2)}} mm</b></div><div class="result-row"><span>衰减</span><b>{{beamAtt.toFixed(2)}} dB</b></div><div class="result-row"><span>运行模式</span><b>{{settingsDraft.beamRunMode==='AUTO'?'自动':'手动'}}</b></div><div class="result-row"><span>束宽算法</span><b>{{settingsDraft.beamWidthMethod==='D4SIGMA'?'D4σ / ISO 11146':'FWHM'}}</b></div><div class="result-row"><span>离群点处理</span><b>{{settingsDraft.beamAutoOutlier?'自动剔除':'关闭'}}</b></div><div class="result-row"><span>X / Y 拟合曲线</span><b>{{settingsDraft.beamShowX?'显示':'隐藏'}} / {{settingsDraft.beamShowY?'显示':'隐藏'}}</b></div></div><div class="result-section"><h4>接口状态</h4><div class="result-row"><span>当前数据源</span><b>{{snapshot.mode==='SIM'?'模拟器':'真实硬件'}}</b></div><div class="result-row"><span>目标设备</span><b>{{interfaceFor('beam')?.deviceName}}</b></div><div class="result-row"><span>厂商软件</span><b>{{interfaceFor('beam')?.vendorSoftware}}</b></div><div class="result-row"><span>通信接口</span><b>{{interfaceFor('beam')?.interfaceName}}</b></div><div class="result-row"><span>接口地址</span><b>{{interfaceFor('beam')?.endpoint}}</b></div><div class="result-row"><span>配置状态</span><b>{{interfaceStateText(interfaceFor('beam')?.state)}}</b></div><p>{{interfaceFor('beam')?.message}}</p></div></div></div></aside>
        ...
        <article class="instrument-panel scope-panel" :class="{hidden:!['dashboard','scope'].includes(activePage)}">
          <div class="module-head scope-head graph-overlay">
            ...
            <template v-if="activePage==='scope'">
              <div class="focus-head-metrics">
                <div v-for="trace in snapshot.scope.time" :key="'scope-head-'+trace.name" class="focus-head-value"><span>{{trace.name}}</span><div><b>{{trace.points.at(-1)?.y.toFixed(3) ?? '0.000'}}</b></div></div>
          ...
          <div v-if="activePage==='scope'" class="focus-summary">
            ...
            <div class="focus-metric"><span>采样率</span><div><b>2.5</b><small>MSa/s</small></div></div>
