...
/// </summary>
internal sealed class UiOverviewMeterTextV036 : IDisposable
{
    private readonly UiShellV032 _shell;
    private readonly List<Binding> _bindings = new();
    private bool _disposed;

    private UiOverviewMeterTextV036(UiShellV032 shell)
    {
        _shell = shell;
        Bind("_coverageText", "_coverageMeter", ContentAlignment.MiddleCenter, 8.2F, suppressPulse: false);
        Bind("_currentText", "_currentMeter", ContentAlignment.MiddleLeft, 8.2F, suppressPulse: true);
        Bind("_uploadText", "_uploadMeter", ContentAlignment.MiddleCenter, 7.4F, suppressPulse: false);
        Bind("_downloadText", "_downloadMeter", ContentAlignment.MiddleCenter, 7.4F, suppressPulse: false);
    }

    public static UiOverviewMeterTextV036 Attach(UiShellV032 shell) => new(shell);

    private void Bind(string labelField, string meterField, ContentAlignment alignment, float fontSize, bool suppressPulse)
    {
        ...
        if (label.Parent is not TableLayoutPanel table || !ReferenceEquals(meter.Parent, table))
            throw new InvalidOperationException($"v0.3.6 meter text expected {labelField} and {meterField} in the same TableLayoutPanel");
        ...
        if (labelCell.Column != meterCell.Column || labelCell.Row < 0 || meterCell.Row != labelCell.Row + 1)
            throw new InvalidOperationException($"v0.3.6 meter text found unexpected source layout for {labelField}/{meterField}");

        var binding = new Binding(label, meter, table, labelCell, meterCell);
    ...
    private T Field<T>(string name) where T : class
    {
        ...
        return value as T ?? throw new InvalidOperationException($"v0.3.6 meter text could not resolve UiShellV032.{name}");
    }
