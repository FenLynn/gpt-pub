...
/// <summary>
/// v0.2.7 dashboard refinement. Presentation only. Migration verification,
/// persistence, quota enforcement and WebDAV safety behavior remain unchanged.
/// </summary>
internal sealed class UiDashboardV027 : IDisposable
{
    ...
    private readonly TransportActionButtonV027 _primary = new() { Width = 112, Height = 38 };
    ...
    private UiDashboardV027(MainForm form, AppHost host)
    {
        _form = form;
        _host = host;
        LoadManifestCache();
    ...
    public static UiDashboardV027 Attach(MainForm form, AppHost host)
    {
        var dashboard = new UiDashboardV027(form, host);
        dashboard.ApplyAll();
    ...
    private Control BuildMainContent()
    {
        ...
        var bottom = new TableLayoutPanel { Dock = DockStyle.Top, AutoSize = true, ColumnCount = 2, Margin = new Padding(0, 14, 0, 0) };
    ...
    private static Control BuildQuotaCell(string name, Label value, GradientMeterBar bar, Padding margin)
    {
        ...
        header.Controls.Add(new Label { Text = name, AutoSize = true, Font = new Font("Segoe UI Semibold", 9F) }, 0, 0);
    ...
    private void UpdateCurrent(UiEffectiveState state)
    {
        ...
        _stageTrack.ActiveIndex = ResolveStage(message, state);
        ...
        {
            var knownSize = FindKnownSize(relative);
            var byteStage = message.Contains("Downloading source", StringComparison.OrdinalIgnoreCase) ||
                            message.Contains("Target already exists", StringComparison.OrdinalIgnoreCase) ||
                            message.Contains("Uploading target", StringComparison.OrdinalIgnoreCase) ||
                            message.Contains("Re-downloading target", StringComparison.OrdinalIgnoreCase);
            _currentBar.Fraction = 0;
            _currentBar.Pulse = !byteStage || knownSize <= 0;
            _currentBar.BarText = byteStage && knownSize > 0 ? $"{fileName}    0%" : fileName;
        }
    }

    private long FindKnownSize(string relative)
    {
        if (_host.State.Files.TryGetValue(relative, out var direct) && direct.SourceSize > 0) return direct.SourceSize;
        var name = Path.GetFileName(relative);
        return _host.State.Files.Values.FirstOrDefault(x => string.Equals(Path.GetFileName(x.RelativePath), name, StringComparison.OrdinalIgnoreCase))?.SourceSize ?? 0;
    }
    ...
    {
        var ratio = (double)Math.Max(0, used) / Math.Max(1, total);
        bar.Fraction = Math.Clamp(ratio, 0, 1);
    ...
    private void UpdatePrimary(UiEffectiveState state)
    {
        ...
        switch (state)
        {
            case UiEffectiveState.Running:
                _primary.SetAction(TransportActionKindV027.Pause, "暂停");
                break;
            case UiEffectiveState.Paused:
                _primary.SetAction(TransportActionKindV027.Play, "继续");
                break;
            case UiEffectiveState.Preparing:
                _primary.SetAction(TransportActionKindV027.Pause, "暂停");
                break;
            case UiEffectiveState.Pausing:
                _primary.SetAction(TransportActionKindV027.Busy, "正在暂停");
                break;
            case UiEffectiveState.WaitNetwork:
                _primary.SetAction(TransportActionKindV027.None, "等待网络");
                break;
            case UiEffectiveState.WaitQuota:
                _primary.SetAction(TransportActionKindV027.None, "等待新周期");
                break;
            case UiEffectiveState.WaitRetry:
                _primary.SetAction(TransportActionKindV027.None, "需要处理");
                break;
            case UiEffectiveState.Complete:
                _primary.SetAction(TransportActionKindV027.None, "已完成");
                break;
            default:
                _primary.SetAction(TransportActionKindV027.Play, "继续");
                break;
    ...
    private string FormatRecent()
    {
        ...
        if (age < TimeSpan.FromMinutes(1)) return "最近完成 1 分钟内";
        if (age < TimeSpan.FromHours(1)) return $"最近完成 {Math.Max(1, (int)age.TotalMinutes)} 分钟前";
...
internal enum TransportActionKindV027
{
    None,
    Play,
    Pause,
...
internal sealed class TransportActionButtonV027 : Control
{
    private bool _hover;
    ...
    public TransportActionButtonV027()
    {
    ...
    public void SetAction(TransportActionKindV027 kind, string label)
    {
        _kind = kind;
        _label = label;
        Cursor = Enabled && kind is TransportActionKindV027.Play or TransportActionKindV027.Pause ? Cursors.Hand : Cursors.Default;
        Invalidate();
    }
    ...
    protected override void OnMouseDown(MouseEventArgs e) { if (e.Button == MouseButtons.Left && Enabled) { _pressed = true; Invalidate(); } base.OnMouseDown(e); }
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        var bg = !Enabled ? Color.FromArgb(247, 248, 250)
            : _pressed ? Color.FromArgb(232, 239, 246)
            : _hover ? Color.FromArgb(242, 247, 251)
            : Color.White;
        using var background = new SolidBrush(bg);
        using var border = new Pen(Color.FromArgb(194, 200, 207));
        e.Graphics.FillRectangle(background, rect);
        e.Graphics.DrawRectangle(border, rect);

        var ink = Enabled ? Color.FromArgb(36, 62, 82) : Color.FromArgb(145, 149, 153);
        var iconCenter = new Point(28, Height / 2);
        using var iconBrush = new SolidBrush(ink);
        if (_kind == TransportActionKindV027.Play)
        {
            e.Graphics.FillPolygon(iconBrush, new[]
            {
                new Point(iconCenter.X - 5, iconCenter.Y - 7),
                new Point(iconCenter.X - 5, iconCenter.Y + 7),
                new Point(iconCenter.X + 7, iconCenter.Y)
            });
        }
        else if (_kind == TransportActionKindV027.Pause)
        {
            e.Graphics.FillRectangle(iconBrush, iconCenter.X - 6, iconCenter.Y - 7, 4, 14);
            e.Graphics.FillRectangle(iconBrush, iconCenter.X + 2, iconCenter.Y - 7, 4, 14);
        }
        else if (_kind == TransportActionKindV027.Busy)
        {
            using var pen = new Pen(ink, 2f);
            e.Graphics.DrawArc(pen, iconCenter.X - 7, iconCenter.Y - 7, 14, 14, 30, 250);
        }

        var textRect = new Rectangle(40, 0, Math.Max(1, Width - 46), Height);
