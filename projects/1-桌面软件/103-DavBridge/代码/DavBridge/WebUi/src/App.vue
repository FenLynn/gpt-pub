...
<main class="app-shell">
  ...
  <section v-else-if="tab==='transfer'" class="page transfer-page">
    <div class="page-title"><div><span class="page-kicker">任务池</span><h2>转移</h2><p>变化对象优先修复，新发现对象与原有 backlog 同级进入普通稳定池。</p></div><span class="status-chip">{{ snapshot.routeStatus }}</span></div>
  ...
  <section v-else-if="tab==='recycle'" class="page recycle-page">
    <div class="page-title"><div><span class="page-kicker">人工安全门</span><h2>回收站</h2><p>首次缺失只观察，跨周期仍缺失后才允许人工审查。DavBridge 不自动删除。</p></div></div>
