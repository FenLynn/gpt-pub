...
实机反馈确认，仅仅把 21 项文件清单从 About 移到 Web 二级数据页仍然不够。数据页字号偏小，而且从信息架构上仍把低频维护能力放在 Web 主工作区。
...
About 的“关于”Tab 只保留版本、构建和数据维护入口提示。“运行状态”Tab 显示运行、健康、Cycle、会话和最近活动。

数据管理不再具有 Web `tab=data` 页面，也不增加左侧一级导航。数据路径选择、备份恢复和文件清单均由原生 Windows 设置层承担。
