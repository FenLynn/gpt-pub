...
当前实验候选的产品版本为 v0.4.7，位于 `p103-exp`。它尚未提升到 `p103-stable` 或 `main`，也不是正式 Release。
...
```text
55fc61483800e10d871a8406a4ea9608b6d1d0e0
```
...
Windows candidate：`DavBridge-v0.4.7-win-x64`

Artifact ID：`10288835366`

EXE：

```text
2373211 bytes
SHA256 6c345896aa141bd4bf6003bf40f05774c8c1c2ecee06490435b6f0f607816a89
```

Artifact ZIP SHA256：

```text
fac6ad5985a1dded3d1614db9a08e303d34db4ead9a876a08d55b77a5aa83cbf
```
...
本 HANDOFF 更新发生在该代码 head 之后，因此新对话必须把 `55fc6148...` 识别为最后完整验证的代码基线，而不是把后续纯文档提交误当成新的代码候选。
...
用户已确认总览页视觉质量良好。`55fc6148...` 在 UI 与交互冻结轮基础上继续压缩内嵌设置页：四个分类的说明直接挂在顶部分类标签悬浮，不再单独占一整行；安全与维护各项说明直接挂在项目名称本身，取消独立 `ⓘ` 图标；移除重复的“维护工具”小标题；同时收紧顶部分类区、内容区、字段行距、输入框高度和底部按钮区，使设置页在不牺牲可读性的情况下明显减少纵向堆叠。该 head 已通过完整 P103 CI。
...
最新视觉修正进一步解决阶段行的真实布局冲突。旧 `styles.css` 中 `.phase strong { align-self:end; }` 在新 flex 阶段布局中仍然生效，导致“源端对账 / 变化修复 / 等待周期”文字整体落在圆形图标下方。`55fc6148...` 明确覆盖为垂直居中，并移除此前 1 px translate 补偿。CI 的 1100×620 预览测量中，三组圆形图标中心均为 y=166.5 px，三组文字像素中心也均为 y=166.5 px。
...
第六，主窗口默认外框尺寸从 1100×620 改为 `1100×825`，即 16:12 / 4:3。宽度保持不变，只增加高度。历史 `window.json` 若恰好是旧默认 1100×620，会自动迁移到新默认高度；用户自己调整过的其他尺寸不会被覆盖。恢复窗口时还会限制在当前显示器工作区内。
...
用户下一步需要在真实 Windows 上运行 `55fc6148...` 对应的 v0.4.7 candidate，重点检查：

1. 默认窗口应为 1100×825，视觉比例约 16:12；如果本机保存的旧窗口恰好是 1100×620，应自动升高到新默认尺寸。
2. 首页“流量预算”重置日期后应出现轻量“校准”入口；点击后应进入原生流量校准流程。
3. 设置 → 流量与限速中应直接看到“本周期流量校准”，安全与维护中不再重复显示。
4. 正在迁移时启动校准必须先要求安全暂停；未暂停完成前不得修改额度账本。
5. 校准窗口的上传已用、下载已用和下一重置日期必须正确回填现有基线，保存后首页百分比、重置日期和 Cycle 逻辑应同步更新。
6. 1100×825 下继续检查总览阶段中线、InfiniCLOUD/坚果云路线、转移页队列、设置页纵向空间与 About 页运行会话，不得出现因增高窗口产生的错位或异常拉伸。
