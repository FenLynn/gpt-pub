...
当前实验候选的产品版本为 v0.4.8，位于 `p103-exp`。它尚未提升到 `p103-stable` 或 `main`，也不是正式 Release。
...
Windows candidate：`DavBridge-v0.4.8-win-x64`

Artifact ID：`10295400130`

EXE：

```text
2405979 bytes
SHA256 01e6b6c232fd246e4f0db299e3cdac105a276ee6dde0fab1d0f8f596dfdd35b0
```

Artifact ZIP SHA256：

```text
1b815ca28871dec6e88ceff0e7952093a2de01fc2aec14d374924e3f0451c4d1
```
...
用户下一步需要在真实 Windows 上运行 `f10ca58b...` 对应的 v0.4.8 candidate，重点检查：

1. 第一次从旧版本启动 v0.4.8 时，旧无 schema 的窗口记录应一次性迁移到 4:3。典型旧宽度 1100 时应看到约 1100×825，而不是继续恢复成旧扁窗口。
2. 迁移后的 `window.json` 会带 `SchemaVersion=2`。此后用户自己拖动的新尺寸应被尊重，不应每次启动再次强制 4:3。
3. 首页流量预算中，标题、重置日期、校准按钮与上传/下载数据应形成紧凑一组；特别检查上传/下载上方不再出现 v0.4.7 那种大块空白。
4. 首页的额外纵向空间应主要表现为模块之间和页面底部的呼吸空间，不允许把镜像覆盖、流量预算或当前任务内部无意义拉高。
5. 首页“校准”与“设置 → 流量与限速 → 本周期流量校准”都必须进入同一原生校准流程；运行中必须先安全暂停。
6. 总览阶段中线、InfiniCLOUD/坚果云路线、第二页队列、设置页、单实例、睡眠/网络恢复和无桌面 BalloonTip 行为不得回退。
